...
1) Compute residuals on the test set for the best hybrid model.
2) Identify listings where hybrid performance is strongest vs baseline proxy.
3) Extract high-value vs low-value lexical signals from train-set quartiles.
4) Generate a bar chart of discriminative words.
"""
...
from features.text import Vocabulary, simple_tokenize  # noqa: E402
...
def _load_vocab(city: str) -> Vocabulary:
    """Load vocabulary generated by the text model training."""
    vocab_path = RESULTS_DIR / city / "text_model" / "vocab.json"
    if not vocab_path.exists():
        raise FileNotFoundError(
            f"Missing vocabulary file: {vocab_path}. Run train_text_model.py first."
        )
    with vocab_path.open("r", encoding="utf-8") as input_file:
        return Vocabulary.from_dict(json.load(input_file))


def _load_hybrid_model(city: str, device: torch.device) -> HybridPredictor:
    """Load trained hybrid model checkpoint and instantiate architecture."""
    checkpoint_path = RESULTS_DIR / city / "hybrid" / "best_model.pth"
    if not checkpoint_path.exists():
        raise FileNotFoundError(
            f"Missing hybrid checkpoint: {checkpoint_path}. Run train_hybrid_model.py first."
        )

    checkpoint = torch.load(checkpoint_path, map_location=device)
    model_config = checkpoint["model_config"]
    model = HybridPredictor(**model_config).to(device)
...
def _predict_hybrid(
    ...
    """Generate hybrid predictions for the full test set."""
...
def run_interpretability(
    ...
    model = _load_hybrid_model(city, device)

    hybrid_predictions = _predict_hybrid(
        model=model,
    ...
    residual_df["hybrid_prediction"] = hybrid_predictions
    residual_df["hybrid_abs_error"] = np.abs(
        residual_df[TARGET_COLUMN] - residual_df["hybrid_prediction"]
    )
    ...
    if baseline_path.exists():
        ...
        residual_df["hybrid_vs_baseline_gain_proxy"] = (
            baseline_rmse - residual_df["hybrid_abs_error"]
        )
    else:
        residual_df["baseline_rmse_proxy"] = np.nan
        residual_df["hybrid_vs_baseline_gain_proxy"] = np.nan

    top_listings_df = residual_df.sort_values(
        "hybrid_vs_baseline_gain_proxy", ascending=False
    ...
    if DESCRIPTION_COLUMN in top_listings_df.columns:
        ...
        residual_output_columns.append("description_preview")

    top_listings_df[residual_output_columns].to_csv(
        output_dir / "top_hybrid_vs_baseline_listings.csv", index=False
    )
    ...
    summary = {
        ...
        "hybrid_mae": float(residual_df["hybrid_abs_error"].mean()),
        "hybrid_rmse": float(
            np.sqrt(
                np.mean(
                    (residual_df[TARGET_COLUMN] - residual_df["hybrid_prediction"]) ** 2
                )
            )
        ),
